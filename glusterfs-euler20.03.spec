...
# rpmbuild -ta glusterfs-10.5.tar.gz --with asan
...
# rpmbuild -ta glusterfs-10.5.tar.gz --with cmocka
%{?_with_cmocka:%global _with_cmocka --enable-cmocka}

# debug
# if you wish to compile an rpm with debugging...
# rpmbuild -ta glusterfs-10.5.tar.gz --with debug
%{?_with_debug:%global _with_debug --enable-debug}

# epoll
# if you wish to compile an rpm without epoll...
# rpmbuild -ta glusterfs-10.5.tar.gz --without epoll
%{?_without_epoll:%global _without_epoll --disable-epoll}

# fusermount
# if you wish to compile an rpm without fusermount...
# rpmbuild -ta glusterfs-10.5.tar.gz --without fusermount
%{?_without_fusermount:%global _without_fusermount --disable-fusermount}

# geo-rep
# if you wish to compile an rpm without geo-replication support, compile like this...
# rpmbuild -ta glusterfs-10.5.tar.gz --without georeplication
%{?_without_georeplication:%global _without_georeplication --disable-georeplication}

# gnfs
# if you wish to compile an rpm with the legacy gNFS server xlator
# rpmbuild -ta glusterfs-10.5.tar.gz --with gnfs
%{?_with_gnfs:%global _with_gnfs --enable-gnfs}

# ipv6default
# if you wish to compile an rpm with IPv6 default...
# rpmbuild -ta glusterfs-10.5.tar.gz --with ipv6default
%{?_with_ipv6default:%global _with_ipv6default --with-ipv6-default}

# linux-io_uring
# If you wish to compile an rpm without linux-io_uring support...
# rpmbuild -ta  glusterfs-10.5.tar.gz --disable-linux-io_uring
...
%if ( 0%{?fedora} && 0%{?fedora} <= 32 ) || ( 0%{?rhel} && 0%{?rhel} <= 7 ) || ( 0%{?openEuler} && 0%{?openEuler} <= 2 )
...
# rpmbuild -ta glusterfs-10.5.tar.gz --without libtirpc
%{?_without_libtirpc:%global _without_libtirpc --without-libtirpc}

# libtcmalloc
# if you wish to compile an rpm without tcmalloc (i.e. use gluster mempool)
# rpmbuild -ta glusterfs-10.5.tar.gz --without tcmalloc
...
# rpmbuild -ta glusterfs-10.5.tar.gz --without ocf
%{?_without_ocf:%global _without_ocf --without-ocf}

# server
# if you wish to build rpms without server components, compile like this
# rpmbuild -ta glusterfs-10.5.tar.gz --without server
...
# rpmbuild -ta glusterfs-10.5.tar.gz --without syslog
...
# rpmbuild -ta glusterfs-10.5.tar.gz --with tsan
...
# rpmbuild -ta glusterfs-10.5.tar.gz --with valgrind
...
%if ( 0%{?rhel} && 0%{?rhel} <= 6 ) || ( 0%{?openEuler} && 0%{?openEuler} <= 2 )
...
#exclude regression-tests related
...
%exclude %{_sharedstatedir}/glusterd/hooks/1/start/post/S31ganesha-start.sh
